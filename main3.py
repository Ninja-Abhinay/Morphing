...
def morph_faces(img1, img2, alpha=0.5):
    ...
    imgMorph = cv2.cvtColor(imgMorph, cv2.COLOR_BGR2RGB)  # Ensure the final image is in RGB format for Streamlit display
...
def main():
    ...
    if len(uploaded_files) == 2:
        ...
        img1 = cv2.cvtColor(img1, cv2.COLOR_BGR2RGB)  # Convert to RGB for consistent display in Streamlit
        img2 = load_and_prepare_image(uploaded_files[1])
        img2 = cv2.cvtColor(img2, cv2.COLOR_BGR2RGB)  # Convert to RGB for consistent display in Streamlit
        morphed_face = morph_faces(img1, img2, alpha)

        # Display uploaded images and the morphed face
        ...
        with col3:
            st.image(morphed_face, caption="Morphed Face", width=200)
    else:
